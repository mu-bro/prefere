...
$data = array_merge( isset($data) ? $data : array() , $this->load->language('account/order'));
		...
		if ($order_info) {
			$data['heading_title'] = sprintf($this->language->get('text_order_number'), $this->request->get['order_id']);
			...
			$data['back_href'] = $this->url->link('account/order', $url, 'SSL');
...
//p($data['products']);
			...
			foreach ($totals as $total) {
				...
				if ($total['code'] == 'total') {
					$total_array['last'] = true;
				}
				$data['totals'][] = $total_array;
			}
