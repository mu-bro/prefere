...
class ControllerPaymentPayanywayComepay extends Controller {
	public function index() {
		...
		$data['paymentSystemUnitId'] = 228820;
		$data['paymentSystemAccountId'] = 47654606;
		$data['mnt_success_url'] = $this->url->link('payment/payanyway/success');
		$data['mnt_fail_url'] = $this->url->link('checkout/checkout', '', 'SSL');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_comepay.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/payanyway_comepay.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/payanyway_comepay.tpl', $data);
		}
	}
