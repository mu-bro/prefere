...
class ControllerPaymentPayanywayContact extends Controller {
	public function index() {
		...
		$data['paymentSystemUnitId'] = 1028;
		$data['paymentSystemAccountId'] = 26;
		$data['mnt_success_url'] = $this->url->link('payment/payanyway/success');
		$data['mnt_fail_url'] = $this->url->link('checkout/checkout', '', 'SSL');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_contact.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/payanyway_contact.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/payanyway_contact.tpl', $data);
		}
	}
