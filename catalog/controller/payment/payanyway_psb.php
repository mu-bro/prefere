...
class ControllerPaymentPayanywayPsb extends Controller {
	public function index() {
		...
		$data['paymentSystemUnitId'] = 661709;
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_psb.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/payanyway_psb.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/payanyway_psb.tpl', $data);
		}
	}
