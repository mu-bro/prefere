...
class ControllerPaymentPayanywayMM extends Controller {
	public function index() {
		...
		$data['entry_moneymail_email'] = $this->language->get('entry_moneymail_email');
		$data['entry_moneyMailError'] = $this->language->get('entry_moneyMailError');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_mm.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/payanyway_mm.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/payanyway_mm.tpl', $data);
		}
	}
