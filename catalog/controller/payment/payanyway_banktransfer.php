...
class ControllerPaymentPayanywayBanktransfer extends Controller {
	public function index() {
		...
		$data['paymentSystemUnitId'] = 705000;
		$data['paymentSystemAccountId'] = 75983431;
		$data['mnt_success_url'] = $this->url->link('payment/payanyway/success');
		$data['mnt_fail_url'] = $this->url->link('checkout/checkout', '', 'SSL');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_banktransfer.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/payanyway_banktransfer.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/payanyway_banktransfer.tpl', $data);
		}
	}
