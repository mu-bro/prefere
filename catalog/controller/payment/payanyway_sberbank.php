...
class ControllerPaymentPayanywaySberbank extends Controller {
	public function index() {
		...
		$data['paymentSystemUnitId'] = 510801;
		$data['paymentSystemAccountId'] = 160;
		$data['mnt_success_url'] = $this->url->link('payment/payanyway/success');
		$data['mnt_fail_url'] = $this->url->link('checkout/checkout', '', 'SSL');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/payanyway_sberbank.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/payanyway_sberbank.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/payanyway_sberbank.tpl', $data);
		}
	}
