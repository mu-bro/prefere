...
class ModelCheckoutOrder extends Model {
    public function addOrder($data) {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET invoice_prefix = '" . $this->db->escape($data['invoice_prefix']) . "', store_id = '" . (int)$data['store_id'] . "', store_name = '" . $this->db->escape($data['store_name']) . "', store_url = '" . $this->db->escape($data['store_url']) . "', customer_id = '" . (int)$data['customer_id'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "',
        ...
        if (isset($data['products'])) {
            foreach ($data['products'] as $product) {
                ...
                del_info = '" . $this->db->escape(serialize($product['delInfo'])) . "',
    ...
    public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
        ...
        if ($order_info) {
            ...
            if (!$order_info['order_status_id'] && $order_status_id) {
                ...
                $data['title'] = $language->get('text_new_order_detail');
                ...
                $data['order_comment'] = nl2br($order_info['comment']);
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/invoice_template.tpl')) {
                    $html = $this->load->view($this->config->get('config_template') . '/template/mail/invoice_template.tpl', $data);
                } else {
                    $html = $this->load->view('default/template/mail/invoice_template.tpl', $data);
                }
                ...
                $mail->setHtml($this->document->createMail($html));
                $mail->setText($text);
                $mail->send();
                ...
                if ($this->config->get('config_order_mail')) {
                    $subject = $language->get('text_new_received') . " " . sprintf($language->get('text_new_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'), $order_id);
            ...
            if ($order_info['order_status_id'] && $order_status_id && $notify) {
                ...
                $mail->setHtml($this->document->createMail($message));
                $mail->send();
            }
        }
