...
$(function () {
    $("#slider-range").slider({range:true, min:0, max:0, values:[0, 0], stop:function (a, b) {
        ...
        setPrice(b.values[0], b.values[1]);
    }});
        ...
        if (g.totals_data) {
            ...
            $.each(g.totals_data.manufacturers, function (f, k) {
                if (k.id) {
                    ...
                    if (j.length && j.get(0).tagName == "OPTION") {
            ...
            $(".option_value").each(function (j, k) {
                ...
                if (idValue != undefined) {
                    var l = idValue.match(/option_value_(\d+)/);
                    if ($.inArray(l[1], e) < 0) {
...
$(document).ready(function () {
    ...
    $("#filterpro .filtered").each(function() {
        changeLabelClass(this);
    });
    if ($(".sort select").length) {
...
function setPrice(min, max) {
    $("#min_price").val(min);
    $("#max_price").val(max);
    $('#slider-range .ui-state-default').html(min);
    $('#slider-range .ui-state-default:last-child').html(max);
}
function changeLabelClass(item, checkDisabled = false) {
