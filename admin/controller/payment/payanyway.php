...
class ControllerPaymentPayanyway extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('moneta', $this->request->post['moneta']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);

		$data['action'] = $this->url->link('payment/payanyway', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['moneta_username'])) {
			$data['moneta_username'] = $this->request->post['moneta_username'];
		} else {
			$data['moneta_username'] = $this->config->get('moneta_username');
		}

		if (isset($this->request->post['moneta_password'])) {
			$data['moneta_password'] = $this->request->post['moneta_password'];
		} else {
			$data['moneta_password'] = $this->config->get('moneta_password');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway.tpl', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway')) {
