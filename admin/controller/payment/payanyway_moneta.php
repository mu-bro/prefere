...
class ControllerPaymentPayanywayMoneta extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_moneta', $this->request->post['payanyway_moneta']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_moneta', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

		$data['action'] = $this->url->link('payment/payanyway_moneta', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_moneta_mnt_test_mode'])) {
		...
		if (isset($this->request->post['payanyway_moneta_status'])) {
			$data['payanyway_moneta_status'] = $this->request->post['payanyway_moneta_status'];
		} else {
			$data['payanyway_moneta_status'] = $this->config->get('payanyway_moneta_status');
		}

		if (isset($this->request->post['payanyway_moneta_sort_order'])) {
			$data['payanyway_moneta_sort_order'] = $this->request->post['payanyway_moneta_sort_order'];
		} else {
			$data['payanyway_moneta_sort_order'] = $this->config->get('payanyway_moneta_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_moneta.tpl', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_moneta')) {
