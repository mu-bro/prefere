...
class ControllerPaymentPayanywayQiwi extends Controller {
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_qiwi', $this->request->post['payanyway_qiwi']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_qiwi', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('payment/payanyway_qiwi', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_qiwi_status'])) {
			$data['payanyway_qiwi_status'] = $this->request->post['payanyway_qiwi_status'];
		} else {
			$data['payanyway_qiwi_status'] = $this->config->get('payanyway_qiwi_status');
		}

		if (isset($this->request->post['payanyway_qiwi_sort_order'])) {
			$data['payanyway_qiwi_sort_order'] = $this->request->post['payanyway_qiwi_sort_order'];
		} else {
			$data['payanyway_qiwi_sort_order'] = $this->config->get('payanyway_qiwi_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_qiwi.tpl', $data));
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_qiwi')) {
