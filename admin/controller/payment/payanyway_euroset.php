...
class ControllerPaymentPayanywayEuroset extends Controller {
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_euroset', $this->request->post['payanyway_euroset']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_euroset', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('payment/payanyway_euroset', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_euroset_status'])) {
			$data['payanyway_euroset_status'] = $this->request->post['payanyway_euroset_status'];
		} else {
			$data['payanyway_euroset_status'] = $this->config->get('payanyway_euroset_status');
		}

		if (isset($this->request->post['payanyway_euroset_sort_order'])) {
			$data['payanyway_euroset_sort_order'] = $this->request->post['payanyway_euroset_sort_order'];
		} else {
			$data['payanyway_euroset_sort_order'] = $this->config->get('payanyway_euroset_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_euroset.tpl', $data));
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_euroset')) {
