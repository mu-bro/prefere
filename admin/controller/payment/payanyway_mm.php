<?php 
class ControllerPaymentPayanywayMM extends Controller {
	private $error = array(); 
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_mm', $this->request->post['payanyway_mm']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_mm', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('payment/payanyway_mm', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_mm_status'])) {
			$data['payanyway_mm_status'] = $this->request->post['payanyway_mm_status'];
		} else {
			$data['payanyway_mm_status'] = $this->config->get('payanyway_mm_status');
		}

		if (isset($this->request->post['payanyway_mm_sort_order'])) {
			$data['payanyway_mm_sort_order'] = $this->request->post['payanyway_mm_sort_order'];
		} else {
			$data['payanyway_mm_sort_order'] = $this->config->get('payanyway_mm_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_mm.tpl', $data));
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_mm')) {
