<?php 
class ControllerPaymentPayanywaySberbank extends Controller {
	private $error = array(); 
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_sberbank', $this->request->post['payanyway_sberbank']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_sberbank', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('payment/payanyway_sberbank', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_sberbank_status'])) {
			$data['payanyway_sberbank_status'] = $this->request->post['payanyway_sberbank_status'];
		} else {
			$data['payanyway_sberbank_status'] = $this->config->get('payanyway_sberbank_status');
		}

		if (isset($this->request->post['payanyway_sberbank_sort_order'])) {
			$data['payanyway_sberbank_sort_order'] = $this->request->post['payanyway_sberbank_sort_order'];
		} else {
			$data['payanyway_sberbank_sort_order'] = $this->config->get('payanyway_sberbank_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_sberbank.tpl', $data));
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_sberbank')) {
