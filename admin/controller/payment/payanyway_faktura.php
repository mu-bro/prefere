<?php
class ControllerPaymentPayanywayFaktura extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_faktura', $this->request->post['payanyway_faktura']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_faktura', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

		$data['action'] = $this->url->link('payment/payanyway_faktura', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_faktura_mnt_test_mode'])) {
		...
		if (isset($this->request->post['payanyway_faktura_status'])) {
			$data['payanyway_faktura_status'] = $this->request->post['payanyway_faktura_status'];
		} else {
			$data['payanyway_faktura_status'] = $this->config->get('payanyway_faktura_status');
		}

		if (isset($this->request->post['payanyway_faktura_sort_order'])) {
			$data['payanyway_faktura_sort_order'] = $this->request->post['payanyway_faktura_sort_order'];
		} else {
			$data['payanyway_faktura_sort_order'] = $this->config->get('payanyway_faktura_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_faktura.tpl', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_faktura')) {
