<?php
class ControllerPaymentPayanywayAlfaclick extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('payment/payanyway_alfaclick');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_alfaclick', $this->request->post['payanyway_alfaclick']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_alfaclick', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);

		$data['action'] = $this->url->link('payment/payanyway_alfaclick', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_alfaclick_mnt_test_mode'])) {
		...
		if (isset($this->request->post['payanyway_alfaclick_status'])) {
			$data['payanyway_alfaclick_status'] = $this->request->post['payanyway_alfaclick_status'];
		} else {
			$data['payanyway_alfaclick_status'] = $this->config->get('payanyway_alfaclick_status');
		}

		if (isset($this->request->post['payanyway_alfaclick_sort_order'])) {
			$data['payanyway_alfaclick_sort_order'] = $this->request->post['payanyway_alfaclick_sort_order'];
		} else {
			$data['payanyway_alfaclick_sort_order'] = $this->config->get('payanyway_alfaclick_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_alfaclick.tpl', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_alfaclick')) {
