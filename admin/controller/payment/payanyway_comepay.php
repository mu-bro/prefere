<?php
class ControllerPaymentPayanywayComepay extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_comepay', $this->request->post['payanyway_comepay']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_comepay', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);

		$data['action'] = $this->url->link('payment/payanyway_comepay', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_comepay_status'])) {
			$data['payanyway_comepay_status'] = $this->request->post['payanyway_comepay_status'];
		} else {
			$data['payanyway_comepay_status'] = $this->config->get('payanyway_comepay_status');
		}

		if (isset($this->request->post['payanyway_comepay_sort_order'])) {
			$data['payanyway_comepay_sort_order'] = $this->request->post['payanyway_comepay_sort_order'];
		} else {
			$data['payanyway_comepay_sort_order'] = $this->config->get('payanyway_comepay_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_comepay.tpl', $data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_comepay')) {
