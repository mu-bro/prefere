...
class ControllerPaymentPayanywayNovoplat extends Controller {
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_novoplat', $this->request->post['payanyway_novoplat']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_novoplat', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('payment/payanyway_novoplat', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_novoplat_status'])) {
			$data['payanyway_novoplat_status'] = $this->request->post['payanyway_novoplat_status'];
		} else {
			$data['payanyway_novoplat_status'] = $this->config->get('payanyway_novoplat_status');
		}

		if (isset($this->request->post['payanyway_novoplat_sort_order'])) {
			$data['payanyway_novoplat_sort_order'] = $this->request->post['payanyway_novoplat_sort_order'];
		} else {
			$data['payanyway_novoplat_sort_order'] = $this->config->get('payanyway_novoplat_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_novoplat.tpl', $data));
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_novoplat')) {
