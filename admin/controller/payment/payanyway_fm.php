<?php 
class ControllerPaymentPayanywayFM extends Controller {
	private $error = array(); 
	...
	public function index() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_setting_setting->editSetting('payanyway_fm', $this->request->post['payanyway_fm']);
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/payanyway_fm', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$data['action'] = $this->url->link('payment/payanyway_fm', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['payanyway_fm_status'])) {
			$data['payanyway_fm_status'] = $this->request->post['payanyway_fm_status'];
		} else {
			$data['payanyway_fm_status'] = $this->config->get('payanyway_fm_status');
		}

		if (isset($this->request->post['payanyway_fm_sort_order'])) {
			$data['payanyway_fm_sort_order'] = $this->request->post['payanyway_fm_sort_order'];
		} else {
			$data['payanyway_fm_sort_order'] = $this->config->get('payanyway_fm_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('payment/payanyway_fm.tpl', $data));
	}
	
	private function validate() {
		if (!$this->user->hasPermission('modify', 'payment/payanyway_fm')) {
