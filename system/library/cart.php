...
//p($cart_query->rows,$this->session->data);
            foreach ($cart_query->rows as $cart) {
                $this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE cart_id = '" . (int)$cart['cart_id'] . "'");

                // The advantage of using $this->add is that it will check if the products already exist and increaser the quantity if necessary.
                $cart_id = $this->add($cart['product_id'], $cart['quantity'], json_decode($cart['option']), $cart['recurring_id']);
                if (isset($this->session->data['delInfo'][$cart['cart_id']])) {
                    $cart_info = $this->session->data['delInfo'][$cart['cart_id']];
                    $this->session->data['delInfo'][$cart_id] = $cart_info;
                    unset($this->session->data['delInfo'][$cart['cart_id']]);
                }
            }
        }
    }
    ...
    public function getProducts() {
        ...
        foreach ($cart_query->rows as $cart) {
            $productInfo = $this->getProductInfo($cart);
            if ($productInfo !== null) {
    ...
    public function getProductInfoByCartId($cartId) {
        $cart = $this->db->query("SELECT * FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND cart_id = " . (int)$cartId)->row;
        return $this->getProductInfo($cart);
    }
    public function getProductInfo($cart) {
        $stock = true;
        ...
        if ($product_query->num_rows && ($cart['quantity'] > 0)) {
            ...
            return array(
                ...
                'preorder' => $product_query->row['preorder'],
