...
//переадресации для старых страниц---------------------------------------------------------------------------------

$uri = $_SERVER['REQUEST_URI'];

$template = "/index.php?route=information/information&information_id=7";
if($uri==$template){
    header('Location: /spoluprace');
    die();
}
...
$template = "/platba";
if($uri==$template){
    header('Location: /platebni-zpusoby');
    die();
}

$template = "/rozvoz";
if($uri==$template){
    header('Location: /rozvoz-kvetin-praha');
    die();
}

$template = "/delivery";
if($uri==$template){
    header('Location: /dorucovani-kvetin-obchodni-podminky');
    die();
}

$template = "/camera";
if($uri==$template){
    header('Location: /nase-vazane-kytice');
    die();
}

$template = "/wedding-bouquet";
if($uri==$template){
    header('Location: /svatebni-kytice');
    die();
}
